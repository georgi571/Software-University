...
public class _02Selling {
    public static void main(String[] args) {
        ...
        playSelling(scanner, matrix, rowS, colS);
    }
    ...
    private static void playSelling(Scanner scanner, char[][] matrix, int row, int col) {
        ...
        while (true) {
            ...
            if ((row < 0 || row > matrix.length - 1) || (col < 0 || col > matrix[0].length - 1)) {
                ...
                if (Character.isDigit(symbol)) {
                    ...
                    matrix[row][col] = '-';
                    int rowB = getRow(matrix, 'O');
                    int colB = getRowCol(matrix, 'O');
                    row = rowB;
                    col = colB;
                    matrix[row][col] = '-';
                }
    ...
    private static void printSelling(char[][] matrix) {
        for (int i = 0; i < matrix.length; i++) {
